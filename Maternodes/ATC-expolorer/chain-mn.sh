...
declare -a ATC amount
declare -a ATC btc usd

if [[ $# -eq 0 || $# -eq 2 || $# > 3 ]]; then
  echo "Usage:   bash ATC-mnTax.sh <ATC ADDRESS> [<START DATE> <END DATE>]
  exit 1
fi
ADDR=$1
...
##########################################################
echo "## Get MN transaction data from ATC block explorer"
...
do
  ...
  # exclude outgoing and 100000 ATC transaction
...
#############################################################
echo "## Get ATC/BTC data from exchange for each MN payment"
for((i=0;i<$cnt;i++))
do
...
for((i=0;i<$cnt;i++))
do
  tmpres=$(echo "${btc[$i]} * ${ATC[$i]} * ${ATCamount[$i]}" | bc)
  usd[$i]=$tmpres
  if [[ $tmpres == "0" ]]; then
    (( error++ ))
    echo "(ERROR) [$i] Date: ${date[$i]} ${time[$i]}, Amount: $ATCamount[$i]} ATC, Exchange price: ${chanix[$i]} ATC/BTC and ${btc[$i]} BTC/USD, Income: ${usd[$i]} USD"
  else
    totalusd=$(echo "$totalusd + ${usd[$i]}" | bc)
    totalATC=$(echo "$totalATC + ${ATCamount[$i]}" | bc)
    echo "[$i] Date: ${date[$i]} ${time[$i]}, Amount: ${ATCamount[$i]} ATC, Exchange price: ${ATC[$i]} ATC/BTC and ${btc[$i]} BTC/USD, Income: ${usd[$i]} USD"
  fi
done
...
echo "Total ATC Received:  $totalATC ATC"
echo 
