...
class ATCdSSH(object):
    ...
    def apply_new_cfg(self):
        ...
        if not len(self.connections):
            raise Exception('There is no connections to ATC network enabled in the configuration.')
        self.cur_conn_def = self.connections[self.cur_conn_index]
    ...
    def switch_to_next_config(self):
        """
        If there is another ATCd config not used recently, switch to it. Called only when there was a problem
    ...
    def open(self):
        """
        Opens connection to ATC RPC. If it fails, then the next enabled conn config will be used, if any exists.
        ...
        try:
            ...
            while True:
                try:
                    ...
                    # exceptions raised by not likely functioning ATCd node; try to switch to another node
    ...
    def open_internal(self):
        """
        Try to establish connection to ATCd RPC daemon for current connection config.
        ...
        if not self.active:
            if self.cur_conn_def.use_ssh_tunnel:
                # RPC over SSH
                while True:
                    self.ssh = ATCdSSH(self.cur_conn_def.ssh_conn_cfg.host, self.cur_conn_def.ssh_conn_cfg.port,
    ...
    def issynchronized(self):
        if self.open():
            # if connecting to HTTP(S) proxy do not check if ATCd daemon is synchronized
