...
a = Analysis(['src/ATC_masternode_tool.py'],
             pathex=[base_dir],
             binaries=[],
             datas=add_files,
             hiddenimports=[],
...
if os_type == 'darwin':
    app = BUNDLE(exe,
                 name='ATC Masternodes',
                 icon='img/dmt.icns',
                 bundle_identifier=None,
                     info_plist= 'ATCMasternodeTool.exe'
...
if os_type == 'win32':
    ...
    os.system('"C:\\Program Files\\7-Zip\\7z.exe" a %s %s -mx0' % (os.path.join(all_bin_dir, 'ATCMasternodeTool_' + version_str + '.win' + no_bits + '.zip'),  
elif os_type == 'darwin':
    print('Compressing Mac executable')
    os.system('zip -r %s %s' % (os.path.join(all_bin_dir, 'ATCMasternodeTool_' + version_str + '.mac.zip'),  'ATCMasternodeTool.app'))
elif os_type == 'linux':
