...
#------------------------------------------------------------------------------
# Bash script for installing pre-requisite packages for cpp-ATC on a
...
# wearable and SBC platforms covered by doublethink and ATCEmbedded would
# also bring in support for Android, iOS, watchOS, tvOS, Tizen, Sailfish,
# Maemo, MeeGo and Yocto.
#
# ------------------------------------------------------------------------------
# This file is part of cpp-ATC
...
uname -v > /dev/null 2>&1 || { echo >&2 "ERROR - cpp-ATC requires 'uname' to identify the platform."; exit 1; }
...
Darwin)
    case $(sw_vers -productVersion | awk -F . '{print $1"."$2}') in
        10.9)
            echo "Installing cpp-ATC dependencies on OS X 10.9 Mavericks."
            ;;
        10.10)
            echo "Installing cpp-ATC dependencies on OS X 10.10 Yosemite."
            ;;
        10.11)
            echo "Installing cpp-ATC dependencies on OS X 10.11 El Capitan."
            ;;
        10.12)
            echo "Installing cpp-ATC dependencies on macOS 10.12 Sierra."
            echo ""
